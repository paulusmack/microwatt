...
void map(void *ea, void *pa, unsigned long perm_attr)
{
	...
	i = (epn >> 9) & 0x3ff;
	j = epn & 0x1ff;
	ptep = (unsigned long *) (pgdir[i] & 0x00ffffffffffff00);
	if (pgdir[i] == 0) {
		pgdir[i] = 0x8000000000000000 | free_ptr | 9;
		free_ptr += 0x1000;
		ptep = (unsigned long *) (pgdir[i] & 0x00ffffffffffff00);
		for (k = 0; k < 512; ++k)
			ptep[k] = 0;
	}
	ptep[j] = 0xc000000000000000 | ((unsigned long)pa & 0x00fffffffffff000) | perm_attr;
